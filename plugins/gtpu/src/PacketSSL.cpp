...
//														 || (packet[offset + i] >= 97 && packet[offset + i] <= 122)))
			...
			{
				string userIP_resolveIP_key = getDNSKey(msgObj->destIpAddrLong, msgObj->sourceIpAddrLong);
//				printf("\nPacket :: %lu Key:: %s -- IP:: %s -- URL:: %s\n", IPGlobal::t_FrameCount, userIP_resolveIP_key.c_str(), msgObj->ipData->s_addr.c_str(), result.c_str());
				updateDns(userIP_resolveIP_key, msgObj->destIpAddrLong, msgObj->sourceIpAddrLong, std::string(result), msgObj->frTimeEpochNanoSec);
