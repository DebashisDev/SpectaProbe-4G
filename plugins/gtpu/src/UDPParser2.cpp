...
VOID UDPParser::lockSysLogMap()
{
	    pthread_mutex_lock(&mapSysLogLock::lockCount);
	    while (mapSysLogLock::count == 0)
	        pthread_cond_wait(&mapSysLogLock::nonzero, &mapSysLogLock::lockCount);
	    mapSysLogLock::count = mapSysLogLock::count - 1;
	    pthread_mutex_unlock(&mapSysLogLock::lockCount);
}

VOID UDPParser::unLockSysLogMap()
{
    pthread_mutex_lock(&mapSysLogLock::lockCount);
    if (mapSysLogLock::count == 0)
        pthread_cond_signal(&mapSysLogLock::nonzero);
    mapSysLogLock::count = mapSysLogLock::count + 1;
    pthread_mutex_unlock(&mapSysLogLock::lockCount);
}
...
{ 
    ...
    if(GContainer::config->OPERATOR_ID == TATASKYBB) {
		if((msgObj->protoSourcePort == SYSLOG_PORT) && (msgObj->protoDestPort == SYSLOG_PORT)) {
			lockSysLogMap();
			parsePacketSysLog(packet + (UDP_HDR_LEN + 5), msgObj); // Facility: LOCAL1 Reserved (5 Bytes)
			unLockSysLogMap();
		}
    }
#endif
...
//    truncatedFlag = (packet[pos+2] & 0x02) >> 1;			// Truncated Message Flag
    ...
    switch(msgObj->dnsQRFlag)
    {
		...
		case RESPONSE:
			 	 ...
			 	 if (msgObj->dnsResponseCode > 5) // Earlier 26
			 	 {
...
//					        msgObj->dnsQRFlag 			= -1;		// Query Response -> Question=0 and Answer=1
    	...
    	case RESPONSE:
					if(qdcount == 1 && ancount <= IPGlobal::S1U_DNS_ANSWER)
					{
						if(parsePacketDNSQueries((pos + DNS_HDR_LEN), id_pos, msgObj, (const unsigned char *)packet, &retPos))
						{
							if(msgObj->dnsResponseCode == 0) parsePacketDNSAnswers(retPos, msgObj, packet);
						}
						else
						{
//					        msgObj->dnsQRFlag 			= -1;		// Query Response -> Question=0 and Answer=1
...
//  strncpy((char *)msgObj->dnsName, (const char *)url.c_str(), (DNS_LEN - 1));
