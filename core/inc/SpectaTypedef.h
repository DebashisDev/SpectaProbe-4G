...
inline uint8_t *Byte2Hex(uint8_t a1, uint8_t *pBuf)
{
...
inline uint8_t *Long2Hex(ULONG a1, uint8_t *pBuf)
{
	uint16_t * pptbl = (uint16_t*) HexLkp2;
	uint16_t * ppout = (uint16_t*) pBuf;
...
inline uint8_t  Hex2Byte(uint8_t n1, uint8_t n2)
{
	n1=toupper(n1);
	n2=toupper(n2);

	uint8_t b1=(char *) isdigit(n1)?(n1-'0'):(n1-'A'+10);
	uint8_t b2=(char *) isdigit(n2)?(n2-'0'):(n2-'A'+10);

	return (b1<<4)|b2;
}

inline uint8_t * ExtractIP(uint8_t *pszString, uint8_t *pBuf)
{
	const uint8_t * pptr = pszString;
	uint8_t b[4];
