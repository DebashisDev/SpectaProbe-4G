...
VOID
ProbeUtility::Append(uint8_t *original, const uint8_t *add)
{
   while(*original)
      original++;
...
VOID
ProbeUtility::printBytes(const uint8_t *identifier, bool printflag, const uint8_t *packet, uint32_t size)
{
	if(printflag){
...
ProbeUtility::parseTcpTimeStamp(struct tcphdr *tcp, ULONG *tsval, ULONG *tsecr)
{
	...
	tcpHeader = ((uint8_t *)tcp) + sizeof(*tcp);
...
VOID
ProbeUtility::getIPHex(uint8_t *address, uint8_t *hexaddress)
{
	uint8_t *p, a[25];
	uint32_t i = 0;
...
VOID
ProbeUtility::fillIP(uint8_t *address, uint8_t *fillInAddress)
{
	uint8_t *p, a[25];
	uint32_t i = 0;
...
//VOID
//ProbeUtility::flushHTTPData(const uint8_t *sIP, const uint8_t * dIP, uint16_t sP, uint16_t dP, uint8_t *tokenBuf)
//{
...
//VOID
//ProbeUtility::ExtractIP4AddressR(const BYTE packet, uint8_t *ipBuffer, uint32_t loc)
//{
//	unsigned int address;
//
...
VOID
ProbeUtility::ExtractIP6Address(const uint8_t *packet, char *ipBuffer, uint32_t loc)
{
...
VOID
ProbeUtility::ExtractIP6Prefix(const uint8_t *packet, char *ipBuffer, uint32_t start, uint32_t end)
{
	ipBuffer[0] = 0;
