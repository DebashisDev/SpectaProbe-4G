...
void AdminPortReader::run()
{
	...
	while(IPGlobal::PROBE_STATS_RUNNING_STATUS)
	{
		...
		else if(strstr(buffer, "PAUSE_ROUTER") != NULL)
			IPGlobal::ROUTER_PACKET_PROCESSING = false;
		else if(strstr(buffer, "RESUME_ROUTER") != NULL)
			IPGlobal::ROUTER_PACKET_PROCESSING = true;
		else
...
VOID AdminPortReader::refreshConfig()
{
	...
	while(!fp.eof())
	{
		...
		else if(Key.compare("S1U_SESSION_PKT_LIMIT") == 0)
		{
			fp >> Value;
			IPGlobal::S1U_SESSION_PKT_LIMIT = atoi(Value.c_str());
			printf("	S1U_SESSION_PKT_LIMIT			:: %d\n", IPGlobal::S1U_SESSION_PKT_LIMIT);
		}
		else if(Key.compare("S1U_SESSION_TIME_INDEX_PKT_LIMIT") == 0)
		{
			fp >> Value;
			IPGlobal::S1U_SESSION_TIME_INDEX_PKT_LIMIT = atoi(Value.c_str());
			printf("	S1U_SESSION_TIME_INDEX_PKT_LIMIT		:: %d\n", IPGlobal::S1U_SESSION_TIME_INDEX_PKT_LIMIT);
		}
		else if(Key.compare("S1U_SESSION_FLUSH_TIMEOUT_SEC") == 0)
		{
			fp >> Value;
			IPGlobal::S1U_SESSION_TIME_LIMIT = atoi(Value.c_str());
			printf("	S1U_SESSION_FLUSH_TIMEOUT_SEC			:: %d\n", IPGlobal::S1U_SESSION_TIME_LIMIT);
		}
		else if(Key.compare("S1U_SESSION_CLEAN_UP_BATCH_LIMIT") == 0)
		{
			fp >> Value;
		...
		else if(Key.compare("S1U_WRITE_XDR") == 0)
		{
			fp >> Value;
		...
		else if(Key.compare("S1U_SESSION_CLEAN_UP_TIMEOUT_SEC") == 0)
		{
			fp >> Value;
